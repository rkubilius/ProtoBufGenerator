...
var
  _ReservedWords: TStringList;

const
  RESERVED: array[0..74] of string = (
    ...
    'result', 'self', 'private', 'protected', 'public', 'published',
    'automated', 'on', 'absolute', 'abstract'
  );

function IsReservedWord(AStr: string): Boolean;
var
  s: string;
begin
  if _ReservedWords = nil then
  begin
    _ReservedWords := TStringList.Create;
    _ReservedWords.Sorted := True;
    _ReservedWords.CaseSensitive := True;
    for s in RESERVED do
      _ReservedWords.Add(s);
  end;

  Result := _ReservedWords.IndexOf(LowerCase(AStr)) > -1;
end;
...
begin
  ...
  // Add trailing underscore to the names that can make the autogenerated code
  // uncompilable (names that coincide with reserved words, etc.)
  if IsReservedWord(FName) then
    FName := FName + '_';

  // Ensure first capital letter of field name (Pascal style)
